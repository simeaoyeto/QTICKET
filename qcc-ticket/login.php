 ...
 * KIAMI — Autenticação
 *
 * Três modos de login (por ordem de verificação):
 * 1. Área administrativa — username = nome da área (RH, Finanças…); password = nome do solicitante
 * 2. Operação — username = nome da operação (ENSA, QCC…); perfil Operador; redireciona para tickets
 * 3. Utilizador interno — username + password_hash na tabela utilizadores
 *
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($username)) {
        ...
        if (verificarBloqueioLogin($pdo, $username)) {
            ...
            // Mapa: chave = texto digitado no login → valor = id_area na BD
            $areasAdministrativas = [

                'finanças' => 5, 'financas' => 5,
                'rh' => 4,
                ...
                'serviços gerais' => 7, 'servicos gerais' => 7,

                'formação' => 8, 'formacao' => 8,

                'direção' => 3, 'direcao' => 3,

                'auditoria' => 7,

                'formadores' => 8, 'formador' => 8,

            ];

            $areaKey = strtolower($username);



            if (isset($areasAdministrativas[$areaKey])) {

                $nome = !empty($password) ? $password : "Solicitante " . ucfirst($username);

                $idArea = $areasAdministrativas[$areaKey];

                $userId = garantirUtilizadorSessao($pdo, 'area.' . $areaKey, $nome, 'Comum', $idArea, null);



                $_SESSION['user_id'] = $userId;

                $_SESSION['username'] = $areaKey;

                $_SESSION['nome'] = $nome;

                $_SESSION['perfil'] = 'Comum';

                $_SESSION['id_area'] = $idArea;
                ...
                registarAuditoria($pdo, 'Login', "Login área administrativa: $username");
    ...
    <div class="auth-card">
        ...
        <form action="login.php" method="POST">
            ...
            <div style="margin-bottom: 16px;">
                ...
                <label style="display: block; margin-bottom: 6px; color: #cbd5e1; font-size: 13px;">Utilizador, Área ou Operação</label>

                <input type="text" name="username" required placeholder="Ex: admin, ENSA, RH" class="auth-input">
