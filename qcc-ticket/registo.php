 ...
 * - Operador / Supervisão → escolhe a operação (email opcional)
...
$perfisPublicos = [
    ...
    'Supervisao' => 'Supervisão (operação)',
    'Tecnico' => 'Técnico',
            ...
            if ($erro === '' && $destinoOk) {
                try {
                    ...
                    if ($check->fetch()) {
                        ...
                        $emailFinal = $emailOk ? $emailMsg : null;

                        if ($perfil === 'Supervisao' && $idOperacao) {
                            $idAreaSup = obterIdAreaSupervisaoOperacao($pdo, $idOperacao);
                            if ($idAreaSup) {
                                $idArea = $idAreaSup;
                            }
                        }

                        [$idPerfilResolvido] = resolverPerfilPost($pdo, 0, $perfil);

                        $stmt = $pdo->prepare("
                            INSERT INTO utilizadores (nome, username, email, password_hash, perfil, id_perfil, estado, id_area, id_operacao)
                            VALUES (?, ?, ?, ?, ?, ?, 'Pendente', ?, ?)
                        ");
                        $stmt->execute([
                            $nome,
                        ...
                        $rotuloPerfil = $perfil === 'Supervisao' ? 'Supervisão' : $perfil;
                        criarNotificacao(
                            $pdo,
                            null,
                            1,
                            'Nova conta pendente',
                            "Novo pedido de conta ({$rotuloPerfil}): {$nome} (@{$username}). Aceite ou recuse em Gestão de Utilizadores.",
                            null
                        );
        ...
        <?php else: ?>
            <form action="registo.php" method="POST" id="form-registo">
                ...
                <div id="bloco-operacao" style="margin-bottom: 14px; display:none;">
                    ...
                    <small id="hint-operacao" style="display:block; margin-top:6px; color:#94a3b8; font-size:11px;">A operação fica fixa no perfil após a aprovação.</small>
                </div>
...
function atualizarDestinoRegisto() {
    ...
    var usaOp = (perfil === 'Operador' || perfil === 'Supervisao');
    ...
    var hintOp = document.getElementById('hint-operacao');
    ...
    if (usaOp) {
        ...
        hint.textContent = perfil === 'Supervisao'
            ? 'Para Supervisão o email não é obrigatório. Fica ligado à Supervisão da operação escolhida.'
            : 'Para Operador o email não é obrigatório. Pode reportar à Supervisão da operação ou às áreas técnicas.';
        hintOp.textContent = perfil === 'Supervisao'
            ? 'Fica associado à Supervisão desta operação (ex.: Supervisão AFRICELL).'
            : 'A operação fica fixa no perfil após a aprovação.';
