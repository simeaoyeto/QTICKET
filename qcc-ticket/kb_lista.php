...
// Perfis que podem gerir a KB (publicar, editar e eliminar):
// Admin (sempre) + Técnicos e Responsáveis das áreas Redes & Sistemas / Desenvolvimento
$podeGerirKb = ($perfil_usuario === 'Admin')
    || (in_array($perfil_usuario, ['Responsavel', 'Tecnico'], true)
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_publicar'])) {
    if (!$podeGerirKb) {
        ...
        if (!$upload['sucesso']) {
            ...
            $stmt_ins = $pdo->prepare("INSERT INTO kb_artigos (titulo, conteudo, imagem, categoria, id_autor, data_criacao) VALUES (?, ?, ?, ?, ?, NOW())");
            $stmt_ins->execute([$titulo, $conteudo, $upload['caminho'], $categoria, $user_id_num]);
            registarAuditoria($pdo, 'Criação', "Artigo KB criado: $titulo");
    ...
    if (!$podeGerirKb) {
        ...
        if (!$upload['sucesso']) {
            ...
            if ($upload['caminho'] !== null) {
                ...
                $stmt_upd = $pdo->prepare("UPDATE kb_artigos SET titulo = ?, conteudo = ?, categoria = ?, imagem = ? WHERE id = ?");
                $stmt_upd->execute([$titulo, $conteudo, $categoria, $upload['caminho'], $id_artigo]);
            } else {
                // Sem nova imagem — mantém a existente
                $stmt_upd = $pdo->prepare("UPDATE kb_artigos SET titulo = ?, conteudo = ?, categoria = ? WHERE id = ?");
                $stmt_upd->execute([$titulo, $conteudo, $categoria, $id_artigo]);
            }
            registarAuditoria($pdo, 'Alteração', "Artigo KB #$id_artigo editado: $titulo");
...
// =========================================================
// PROCURAR ARTIGOS PUBLICADOS (com filtros opcionais)
// =========================================================
$sqlKb = "
...
<?php if (idUtilizadorNumerico()): ?>
        ...
        <div id="main-content">
            ...
            <div class="card" style="margin-bottom: 20px;">
                ...
                <form action="kb_lista.php" method="GET" style="display: grid; grid-template-columns: 1fr 220px auto auto; gap: 12px; align-items: end;">
                    <div>
                ...
                <?php if ($pesquisaKb !== '' || $categoriaFiltro !== ''): ?>
                ...
                <div style="display: flex; flex-direction: column; gap: 20px;">
                    <?php if (empty($artigos)): ?>
                        <div class="card" style="text-align: center; color: var(--text-muted); padding: 40px;">
                            <?php if ($pesquisaKb !== '' || $categoriaFiltro !== ''): ?>
                                Nenhum artigo encontrado com os filtros seleccionados. Tente outra categoria ou palavra-chave.
                            <?php else: ?>
