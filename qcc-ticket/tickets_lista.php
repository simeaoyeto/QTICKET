...
if (isset($_GET['acao']) && isset($_GET['id'])) {
    ...
    if ($acao === 'assumir' && podeResolverTickets($contexto)) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_criar_ticket'])) {
    ...
    // Operador / Supervisão: força a operação de origem (nunca altera)
    if (perfilUsaOperacao($perfil_usuario) && !empty($meu_id_operacao)) {
    ...
    if (!$upload['sucesso']) {
    ...
    } elseif (!areaDestinoPermitidaAbertura($id_area_destino, $contexto, $pdo)) {
        $mensagem = "<div style='background: rgba(239,68,68,0.2); color: var(--red); padding: 12px; border-radius: var(--radius-sm); margin-bottom: 20px;'>Área de destino não permitida para o seu perfil. Operadores podem abrir para Redes & Sistemas, Desenvolvimento ou Supervisão da operação; Supervisão reporta às áreas técnicas.</div>";
    } elseif (perfilUsaOperacao($perfil_usuario) && empty($meu_id_operacao)) {
        $mensagem = "<div style='background: rgba(239,68,68,0.2); color: var(--red); padding: 12px; border-radius: var(--radius-sm); margin-bottom: 20px;'>A sua conta não tem operação associada. Contacte Redes & Sistemas.</div>";
        ...
        $stmt_cad = $pdo->prepare("
            INSERT INTO tickets (codigo, titulo, nome_solicitante, email_solicitante, descricao, prioridade, impacto, urgencia, estado, anexo, id_criador, id_area_destino, id_operacao_origem, id_tecnico_atribuido, data_criacao, data_limite_sla, data_limite_resposta) 
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, NOW(), ?, ?)
        ");
        ...
        while (true) {
            try {
                $stmt_cad->execute([$codigo, $titulo, $nome_usuario, $email_criador, $descricao, $prioridade, $impacto, $urgencia, $estadoInicial, $upload['caminho'], $user_id_num, $id_area_destino, $id_operacao_ticket, $tecnicoSql, $dataLimite, $dataLimiteResp]);
                break;
            } catch (PDOException $e) {
                // Fallback se colunas novas ainda não existirem
                if (str_contains($e->getMessage(), 'impacto') || str_contains($e->getMessage(), 'urgencia') || str_contains($e->getMessage(), 'data_limite_resposta') || str_contains($e->getMessage(), 'Unknown column')) {
                    $stmt_cad = $pdo->prepare("
        ...
        if (ticketENovaEntradaRedes(['titulo' => $titulo, 'id_area_destino' => $id_area_destino])) {
            try {
                garantirChecklistTicket($pdo, $novoId);
            } catch (PDOException $e) {
                // tabela pode ainda não existir
            }
        }

        registarHistoricoTicket($pdo, $novoId, $user_id_num, 'Criação', "Ticket criado com código $codigo — prioridade $prioridade (impacto $impacto / urgência $urgencia)" . ($tecnicoSql ? " e atribuído ao técnico #$tecnicoSql" : ''));
        if ($tecnicoSql) {
...
// =========================================================
$filtro_raw = $_GET['filtro'] ?? ($_GET['estado'] ?? 'Todos');
$filtro_estado = $filtro_raw;
$filtro_sla = null; // risco | vencido
$mapaFiltroPainel = [
    'abertos' => 'Aberto',
    'progresso' => 'Em Progresso',
    'resolvidos' => 'Resolvido',
    'sla_risco' => '__sla_risco__',
    'sla_vencido' => '__sla_vencido__',
];
if (isset($mapaFiltroPainel[$filtro_raw])) {
    $mapped = $mapaFiltroPainel[$filtro_raw];
    if ($mapped === '__sla_risco__') {
        $filtro_sla = 'risco';
        $filtro_estado = 'Todos';
    } elseif ($mapped === '__sla_vencido__') {
        $filtro_sla = 'vencido';
        $filtro_estado = 'Todos';
    } else {
        $filtro_estado = $mapped;
    }
}
...
if ($filtro_estado === 'Atribuidos') {
    ...
    $params[] = $user_id_num;
} elseif ($filtro_sla !== null) {
    $where_clauses[] = "t.estado <> 'Resolvido'";
...
$query_tickets = "
    ...
    $where_sql
    ORDER BY 
        CASE t.prioridade WHEN 'Crítica' THEN 0 WHEN 'Alta' THEN 1 WHEN 'Média' THEN 2 WHEN 'Baixa' THEN 3 ELSE 4 END, 
        t.id DESC
";
...
if ($filtro_sla !== null) {
    $lista_tickets = array_values(array_filter($lista_tickets, static function (array $t) use ($filtro_sla): bool {
        return estadoSlaTicket($t) === $filtro_sla;
    }));
}
...
$rotuloFiltroActivo = null;
if ($filtro_sla === 'risco') {
    $rotuloFiltroActivo = 'SLA em Risco';
} elseif ($filtro_sla === 'vencido') {
    $rotuloFiltroActivo = 'SLA Vencido';
} elseif ($filtro_estado !== 'Todos' && isset($rotulosFiltro[$filtro_estado])) {
    $rotuloFiltroActivo = $rotulosFiltro[$filtro_estado];
} elseif ($filtro_estado !== 'Todos') {
    $rotuloFiltroActivo = $filtro_estado;
}
...
<?php if (idUtilizadorNumerico()): ?>
        ...
        <div id="main-content">
            <div class="page-header">
                ...
                <?php if ($rotuloFiltroActivo): ?>
                    <p style="margin-top:8px;">
                        <span style="display:inline-block; padding:4px 10px; background:rgba(61,111,255,0.15); color:var(--accent); border-radius:var(--radius-sm); font-size:12px; font-weight:600;">
                            Filtro activo: <?php echo htmlspecialchars($rotuloFiltroActivo); ?>
                        </span>
                        <a href="tickets_lista.php" style="margin-left:10px; font-size:12px; color:var(--text-muted);">Limpar</a>
                    </p>
                <?php endif; ?>
            </div>
            ...
            <div style="display: flex; gap: 10px; margin-bottom: 25px; flex-wrap: wrap;">
                <?php
                $tabsFiltro = [
                    'Todos' => ['label' => 'Todos', 'href' => 'tickets_lista.php', 'activo' => $filtro_estado === 'Todos' && $filtro_sla === null],
                    'Aberto' => ['label' => 'Abertos', 'href' => 'tickets_lista.php?filtro=abertos', 'activo' => $filtro_estado === 'Aberto'],
                    'Em Progresso' => ['label' => 'Em Progresso', 'href' => 'tickets_lista.php?filtro=progresso', 'activo' => $filtro_estado === 'Em Progresso'],
                    'Atribuidos' => ['label' => 'Atribuídos', 'href' => 'tickets_lista.php?estado=Atribuidos', 'activo' => $filtro_estado === 'Atribuidos'],
                    'Reencaminhado' => ['label' => 'Reencaminhados', 'href' => 'tickets_lista.php?estado=Reencaminhado', 'activo' => $filtro_estado === 'Reencaminhado'],
                    'Resolvido' => ['label' => 'Resolvidos', 'href' => 'tickets_lista.php?filtro=resolvidos', 'activo' => $filtro_estado === 'Resolvido'],
                    'sla_risco' => ['label' => 'SLA em Risco', 'href' => 'tickets_lista.php?filtro=sla_risco', 'activo' => $filtro_sla === 'risco'],
                    'sla_vencido' => ['label' => 'SLA Vencido', 'href' => 'tickets_lista.php?filtro=sla_vencido', 'activo' => $filtro_sla === 'vencido'],
                ];
                foreach ($tabsFiltro as $chave => $tab):
                    $activo = !empty($tab['activo']);
                    $n = (int)($contagens[$chave] ?? 0);
                    if ($chave === 'sla_risco' || $chave === 'sla_vencido') {
                        $nLabel = '';
                    } else {
                        $nLabel = ' <span style="opacity:.85; font-size:11px;">(' . $n . ')</span>';
                    }
                ?>
                <a href="<?php echo htmlspecialchars($tab['href']); ?>" style="padding: 8px 16px; background: <?php echo $activo ? 'var(--accent)' : 'var(--bg-sidebar)'; ?>; color:#fff; border-radius: var(--radius-sm); text-decoration:none; font-size:13px; font-weight:500;">
                    <?php echo htmlspecialchars($tab['label']); ?><?php echo $nLabel; ?>
                </a>
                <?php endforeach; ?>
            </div>
            ...
            <div style="display: grid; grid-template-columns: 2fr 1fr; gap: 25px; align-items: start;">
                                ...
                                <tr style="border-bottom: 1px solid var(--border); background: rgba(255,255,255,0.01);">
                                    ...
                                    <td style="padding: 15px;">
                                        <?php 
                                            ...
                                            if ($ticket['prioridade'] === 'Crítica') $p_color = 'var(--red)';
                                            if ($ticket['prioridade'] === 'Alta') $p_color = 'var(--red)';
                                            if ($ticket['prioridade'] === 'Média') $p_color = 'var(--amber)';
                                            if ($ticket['prioridade'] === 'Baixa') $p_color = 'var(--green)';
                                        ?>
                                        <span style="color: <?php echo $p_color; ?>; font-weight: 600;"><?php echo htmlspecialchars($ticket['prioridade']); ?></span>
                                    </td>
                                    <td style="padding: 15px;">
                ...
                <div class="card">
                    ...
                    <form action="tickets_lista.php" method="POST" enctype="multipart/form-data" style="display: flex; flex-direction: column; gap: 15px;">
                        <div>
                            <?php
                            $assuntosAberturaForm = ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_criar_ticket']))
                            ...
                            echo htmlSeletorAssuntoCascata(
                                ...
                                '',
                                'novo',
                                'width:100%; padding: 10px; background: var(--bg-input); border: 1px solid var(--border); color:#fff; border-radius:var(--radius-sm);',
                                true,
                                $assuntosAberturaForm
                            );
                            ?>
                        </div>
                        <div>
                        ...
                        <div>
                            ...
                            $isOperacaoPerfil = perfilUsaOperacao($perfil_usuario);
                            $isOperador = ($perfil_usuario === 'Operador');
                            $isSupervisao = ($perfil_usuario === 'Supervisao');
                            // Operador: default Redes; Supervisão escolhe destino técnico (não a própria área)
                            if ($areaAuto <= 0 && $isOperador) {
                                $areaAuto = 1;
                            }
                            if ($isSupervisao) {
                                $areaAuto = 0;
                            }
                            $operacaoTrancada = $isOperacaoPerfil && !empty($meu_id_operacao);
                            $nomeMinhaArea = '';
                            if ($areaAuto > 0) {
                                ...
                                if ($nomeMinhaArea === '' && !$isOperacaoPerfil) {
                                    try {
                            ...
                            ?>
                            <?php if (!$isOperacaoPerfil && $areaAuto > 0): ?>
                                ...
                                <small style="color: var(--text-muted); font-size: 11px;">RH, Redes &amp; Sistemas, Desenvolvimento, Formadores, Legal, Logística, Direção e restantes áreas.</small>
                            <?php else: ?>
                                ...
                                <small style="color: var(--text-muted); font-size: 11px;">
                                    <?php if ($isOperador): ?>
                                        Pode abrir para <b>Redes &amp; Sistemas</b>, <b>Desenvolvimento</b> ou a <b>Supervisão</b> da sua operação.
                                    <?php elseif ($isSupervisao): ?>
                                        A Supervisão reporta às áreas técnicas: <b>Redes &amp; Sistemas</b> ou <b>Desenvolvimento</b>.
                                    <?php else: ?>
                        ...
                        <div style="display:grid; grid-template-columns:1fr 1fr; gap:12px;">
                            <div>
                                <label style="display:block; margin-bottom:5px; font-size:12px; color:var(--text-secondary);">Impacto</label>
                                <select name="impacto" style="width:100%; padding: 10px; background: var(--bg-input); border: 1px solid var(--border); color:#fff; border-radius:var(--radius-sm);">
                            ...
                            <div>
                                <label style="display:block; margin-bottom:5px; font-size:12px; color:var(--text-secondary);">Urgência (opcional)</label>
                                <select name="urgencia" style="width:100%; padding: 10px; background: var(--bg-input); border: 1px solid var(--border); color:#fff; border-radius:var(--radius-sm);">
                                    <option value="">Automática (pelo assunto/SLA)</option>
                        ...
                        <small style="color: var(--text-muted); font-size: 11px; margin-top:-8px;">A prioridade e o SLA são calculados automaticamente (catálogo + impacto × urgência). Em multi-selecção prevalece o mais crítico.</small>
                        <div>
...
// Mostra o campo de texto livre quando se escolhe "Outros" no assunto (legado)
