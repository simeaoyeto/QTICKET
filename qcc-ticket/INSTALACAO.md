...
| Áreas Legal e Logística | Criadas se ainda não existirem |
...
> Os perfis **Cliente operacional** e **Comum** foram **removidos**. Contas antigas são migradas para **Operador** por `atualizar_banco.php`.
...
1. **Operação** — username = nome da operação (ex: `ENSA`) → sessão **Operador**
2. **Conta na BD** — username + password (estado deve ser `Ativo`)
