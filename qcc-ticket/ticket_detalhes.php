...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['btn_alterar_estado'])) {
    ...
    if (!in_array($novo_estado, $estadosValidos, true)) {
        ...
        // Convidado, Comum e Operador apenas abrem e acompanham — não resolvem
                        ...
                        <div class="card">
                            ...
                            <form action="ticket_detalhes.php?id=<?php echo $ticket_id; ?>" method="POST" style="display: flex; flex-direction: column; gap: 10px;">
                                <div>
                                    ...
                                    <select name="assunto_predefinido" onchange="alternarOutroAssunto(this, 'edit_outro')" style="width:100%; padding: 9px; background: var(--bg-input); border: 1px solid var(--border); color:#fff; border-radius:var(--radius-sm);">
                                        <?php echo opcoesAssuntoTicket($ticket['titulo'] ?? '', $pdo); ?>
                                    </select>
                                    <?php $assuntoEhLivre = !in_array($ticket['titulo'] ?? '', obterAssuntosTicket($pdo), true); ?>
                                    <input type="text" name="assunto_outro" id="edit_outro" value="<?php echo $assuntoEhLivre ? htmlspecialchars($ticket['titulo'] ?? '') : ''; ?>" placeholder="Escreva o assunto" style="width:100%; margin-top:6px; padding: 9px; background: var(--bg-input); border: 1px solid var(--border); color:#fff; border-radius:var(--radius-sm); display: <?php echo $assuntoEhLivre ? 'block' : 'none'; ?>;">
                                </div>
                                <div>
